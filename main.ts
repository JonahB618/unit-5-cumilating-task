function MakeLevel () {
    ...
    List = [tilemap`level1`, tilemap`level4`]
    if (LevelNub == List.length) {
        game.gameOver(true)
    }
    tiles.setCurrentTilemap(List[LevelNub])
    tiles.placeOnRandomTile(mySprite, sprites.dungeon.hazardWater)
    for (let value of tiles.getTilesByType(sprites.dungeon.hazardWater)) {
    ...
    tileUtil.replaceAllTiles(sprites.dungeon.hazardWater, assets.tile`transparency16`)
}
let E: Sprite = null
